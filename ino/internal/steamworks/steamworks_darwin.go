...
var libSteamAPIDylib []byte

func loadDylib() (C.uintptr_t, error) {
	dir, err := os.MkdirTemp("", "")
	if err != nil {
		return 0, err
	}

	path := filepath.Join(dir, "libsteam_api.dylib")
	if err := os.WriteFile(path, libSteamAPIDylib, 0644); err != nil {
		return 0, err
	}
	...
	dylib := C.uintptr_t(uintptr(C.dlopen(cpath, C.RTLD_LAZY)))
	if dylib == 0 {
		return 0, fmt.Errorf("steamworks: openDylib failed: %s", C.GoString(C.dlerror()))
	}
